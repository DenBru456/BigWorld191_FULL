...
class AbstractAuctionHouse:
	...
	def registerAuction( self, seller, item, bidPrice, buyoutPrice, expiry ):
		"""
		Register a new auction and returns an auction ID handle.
	...
	def removeAuctionById( self, auctionID ):
		""" Remove an auction identified by its auction ID. """
		raise NotImplementedError
...
class InMemoryAuctionHouse( AbstractAuctionHouse ):
	"""
	Sample implementation of an in-memory auction house with no persistence.
	"""
	def __init__( self ):
		""" Constructor. Set up a dictionary. """
		AbstractAuctionHouse.__init__( self )
		self._auctions = {}


	def getAuction( self, auctionID ):
		"""
		Retrieve an auction from its Auction ID.

		@param auctionID 	the auction ID
		@return an Auction object, or None if there was no such auction.
		"""
		if not self._auctions.has_key( auctionID ):
			return None
		return self._auctions[ auctionID ]


	def getAuctionIds( self ):
		"""
		@return a list of all registered auction IDs
		"""
		return self._auctions.keys()


	def registerAuction( self, seller, item, bidPrice,
	buyoutPrice, expiry ):
		"""
		Register an auction. Saves it in-memory.

		@param seller 		the seller
		@param item 		the item
		@param bidPrice 	the initial bidding price
		@param buyoutPrice 	the buyout price, or None if there is none
		@param expiry 		the expiry time value
		"""
		auctionID = self._nextAuctionId()
		self._auctions[ auctionID ] = Auction( auctionID = auctionID,
			house = self, seller = seller, item = item,
			bidPrice = bidPrice, buyoutPrice = buyoutPrice, expiry = expiry )
		return auctionID

	def removeAuction( self, auction ):
		""" Removes an auction ."""
		del self._auctions[ auction.auctionID ]

	def _nextAuctionId( self ):
		if len(self._auctions) == 0:
			return 1
		return max( self._auctions.keys() ) + 1
...
class FantasyDemoBidder( Bidder ):
	...
	def __cmp__( self, other ):
		...
		if type( other ) != FantasyDemoBidder:
			# this is greater than anything that isn't a bidder
			return 1
		# defer comparison to database ID long
		return self.databaseID.__cmp__( other.databaseID )
