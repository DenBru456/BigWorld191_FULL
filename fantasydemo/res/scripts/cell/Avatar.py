...
class Avatar( BigWorld.Entity ):
	...
	def closeCombatFinishingMove( self ):
		...
		# And call the step function as if the timer we just canceled
	...
	def doExplodeDamage( self, explodePos, explodeRadius, directHitEntity ):
		...
		# If the projectile did a direct hit, move the source of the explosion up by 1 meter, so
		...
		for entity in entitiesInRange:
			...
			entityCollidePos[1] += 0.5	# Move half a meter up, so we don't get occlude by very small objects
	...
	def teleportTo( self, mb, position, direction ):
		self.teleport( mb, position, direction )
	...
	def tradeCommitNotify( self, success, outItemsLock,
		...
		self.client.tradeCommitNotify( success, outItemsLock,
				outItemsSerials, outGoldPieces, inItemsTypes,
				inItemsSerials, inGoldPieces )
	...
	def itemsLockNotify( self, success, lockHandle, itemsSerials, itemsTypes, goldPieces ):
		...
		if lockHandle == self.tradeOutboundLock:
			if success:
				...
				self.tradeOutboundLock = -1
				self.client.tradeOfferItemDeny( lockHandle )
		else:
			assert success
			self.client.itemsLockNotify( lockHandle, itemsSerials, goldPieces )
	...
	def itemsUnlockNotify( self, success, lockHandle ):
		...
		self.client.itemsUnlockNotify( success, lockHandle )
	...
	def commerceStartRequest( self, sourceID, merchantID ):
		...
		if merchant.modeTarget == Mode.NO_TARGET:
			merchant.commerceStartRequest( self.id )
		else:
			self.client.commerceStartDeny()
	...
	def pickUpResponse( self, success, droppedItemID, itemType, itemSerial ):
		...
		if success:
			...
			self.client.pickUpResponse( False, droppedItemID, 0 )
	...
	def startLoadGeneration( self ):#, level ):
		...
		if not self.loadGenTimer:
			self.loadGenTimer = self.addTimer(
				random.randrange(1,10)*0.1, 1.0, Avatar.LOADGEN_TIMER );
	...
	def doLoadGeneration( self ):
		...
		if ((int(BigWorld.time()) + self.id) % Avatar.loadGenPeriod) != 0:
			return
	...
	def getPlayerInfoBaseRequest( self ):
		"""
		A method from the base requesting information about the player.
		See base/Avatar.py/onPlayerInfo.
		"""
		self.base.onPlayerInfo( self.playerName, self.position,	self.direction,
			self.health, self.maxHealth, self.frags )

	def summonEntity( self, sourceID, typeName ):
		assert sourceID == self.id

		direction = self.direction

		try:
			entity = BigWorld.createEntity( typeName,
											self.spaceID,
											self.position,
											direction,
											{} )
	...
	def captureCellBounds( self ):
		...
		for ( childType, label, value ) in dirs:
			if childType is 3:
				...
				for i in rect.split(', '):
					cellBounds.append(float(i))
		self.cellBounds = cellBounds
		self.cellAppID = int(BigWorld.getWatcher( "cellAppID" ))
