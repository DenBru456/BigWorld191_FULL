...
class Creature( BigWorld.Entity ):
	...
	def prerequisites( self ):
		...
		if CreatureData.hideSFX.has_key( t ):
			list += sfx.prerequisites( CreatureData.hideSFX[t][0] )
			list += sfx.prerequisites( CreatureData.hideSFX[t][1] )
		if CreatureData.hitSFX.has_key( t ):
			list += sfx.prerequisites( CreatureData.hitSFX[t] )
		return list
	...
	def onEnterWorld( self, prereqs ):
		...
		#
		# Add the creature as a shadow casting entity
		#
		...
		t = self.creatureType
		if CreatureData.hideSFX.has_key(t):
			self.hideSFX = sfx.OneShotSFX(CreatureData.hideSFX[t][0],10.0,self.prereqs)
			self.revealSFX = sfx.OneShotSFX(CreatureData.hideSFX[t][1],10.0,self.prereqs)

		if CreatureData.hitSFX.has_key(t):
			self.hitSFXName = CreatureData.hitSFX[t]
	...
	def onLeaveWorld( self ):
		#
		# Remove creature as a shadow casting entity
		#
	...
	def setupActionMatcher( self ):
		...
		if self.creatureType == CreatureData.CHICKEN:
			am.entityCollision = 0
			am.collisionRooted = 0
		else:
			if self.creatureState == CreatureData.ALIVE:
				am.entityCollision = 1
				am.collisionRooted = 1
			else:
				am.entityCollision = 0
				am.collisionRooted = 1

		if self.creatureType == CreatureData.SPIDER or \
			self.creatureType == CreatureData.CRAB:
			am.turnModelToEntity = True
			am.useEntityPitchAndRoll = True
		else:
			am.turnModelToEntity = False
			am.useEntityPitchAndRoll = False
