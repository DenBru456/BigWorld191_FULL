...
class Creature( BigWorld.Entity ):
	...
	def onLeaveTrap( self, pEntity, trapRange, trapID ):
		...
		# Have left flee area so cancel fleeing action and choose another
		if self.fleeing and self.ownerId == pEntity.id:
			self.fleeing = 0
			self.ownerId = 0
	...
	def onEnterTrap( self, pEntity, trapRange, trapID ):
		...
		if pEntity.__class__ == Avatar.Avatar and self.ownerId == 0:
			self.cancelCurrent()
			self.fleeing = 1
			self.ownerId = pEntity.id
			self.fleeEntity()


	def fleeEntity( self ):

		# Calculate vector directly away from player
		fleeVector = self.position - BigWorld.entities[self.ownerId].position
		fleeVector.normalise()

		fleeRandomisation = CreatureData.FLEE_RANGE / 6

		# Minimum flee distance must be outside of flee range
		fleePos = self.position + fleeVector.scale( CreatureData.FLEE_RANGE + fleeRandomisation )

		try:
			fleePos = BigWorld.findRandomNeighbourPoint( self.spaceID, fleePos, fleeRandomisation )
		except ValueError, e:
			this.doWait( 0.1 )
			return

		# Navigate to position
		self.navigate( fleePos, self.runVelocity() )

	def onNavigate( self, controllerID, userData ):
		if self.fleeing:
			self.fleeEntity()
		else:
			# Choose new action after fleeing
			self.think()

	def onNavigateFailed( self, controllerID, userData ):
		# If we can't navigate, pick another spot
		self.onNavigate( controllerID, userData )
	...
	def think(self):
		...
		if self.creatureState == CreatureData.DEAD or \
			self.creatureState == CreatureData.DEAD_GIBBED or \
			self.fleeing:
			return
	...
	# Run for your LIVES!
	#------------------------------------------------------------------------
	def doFlee(self, enemyPos, scared):
		self.cancelCurrent()
		...
		pos = (self.position[0] + xd, 0, self.position[2] + zd)
		self.navigate(pos, self.runVelocity(), 5)
