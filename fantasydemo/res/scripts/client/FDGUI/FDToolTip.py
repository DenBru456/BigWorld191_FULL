...
class FDGUIOneLineToolTip( ToolTip ):
	...
	def doLayout( self, parent ):
		...
		self.component.width = (totalLength + textGap + shortcutGap) * scalex
		...
		self.component.height = totalHeight
...
class FDToolTipManager( ToolTipManager ):
	...
	def __init__( self, rootGUI, zorder ):
		...
		self.addToolTipTemplate( "tooltip3line", "gui/tooltip_3line_window.gui" )
		self.addToolTipTemplate( "tooltip2line", "gui/tooltip_2line_window.gui" )
	...
	def readInActionToolTips( self, dataSection ):
		for key, section in dataSection.items():
			...
			templateName = "tooltip1line"#"tooltip3line" if numLines > 2 else "tooltip2line"
		...
		for actionName, toolTipInfo in self.actionToolTips.iteritems():
			...
			if len( bindings ) > 0:
				...
				for key in modifiers:
					if shortcut:
						shortcut += '+'
					shortcut += displayNames[ key ] if displayNames.has_key( key ) else BigWorld.keyToString( key )
				for key in nonmodifiers:
					if shortcut:
						shortcut += '+'
					shortcut += displayNames[ key ] if displayNames.has_key( key ) else BigWorld.keyToString( key )

				toolTipInfo.infoDictionary[ 'shortcut' ] = '(' + shortcut + ')'
			else:
				toolTipInfo.infoDictionary[ 'shortcut' ] = ''
