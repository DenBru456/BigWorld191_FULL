...
class SliderVisualState( VisualState ):
	...
	def onSave( self, dataSection ):
		...
		if self.backgroundTextureMapping:
			...
			self._writeMappingSection( mappingSection, self.backgroundTextureMappingType,
										self.backgroundTextureMapping )
		...
		if self.thumbTextureMapping:
			...
			self._writeMappingSection( mappingSection, self.thumbTextureMappingType,
										self.thumbTextureMapping )
	...
	def apply( self, componentScript ):
		...
		if hasattr( componentScript, "sliderBackground" ):
			...
			if self.thumbTextureMapping:
				applyMapping( componentScript.sliderBackground,
								self.backgroundTextureMappingType,
								self.backgroundTextureMapping )
		...
		if hasattr( componentScript, "sliderThumb" ):
			...
			if self.thumbTextureMapping:
				applyMapping( componentScript.sliderThumb,
								self.thumbTextureMappingType,
								self.thumbTextureMapping )
...
class SliderThumb( PyGUIBase, DraggableComponent ):
	...
	def handleMouseButtonEvent( self, comp, key, down, modifiers, pos ):
		PyGUIBase.handleMouseButtonEvent( self, comp, key, down, modifiers, pos )
	...
	def handleMouseEnterEvent( self, comp, pos ):
		PyGUIBase.handleMouseEnterEvent( self, comp, pos )

		slider = self.component.parent.script
		slider.handleMouseEnterEvent( comp, pos )
...
class Slider( PyGUIBase, VisualStateComponent ):
	...
	def handleMouseButtonEvent( self, comp, key, down, modifiers, pos ):
		if down:
			self.component.thumb.script._setValueFromMouse( pos )
			# Call this to start dragging the thumb, now that it's been moved into position.
			self.component.thumb.script.handleMouseButtonEvent( self.component.thumb, key, down, modifiers, pos )
		return True
