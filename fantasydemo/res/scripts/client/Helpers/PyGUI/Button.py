...
def _getRadioButtons( comp, groupName ):
	if comp is None:
		...
		return [ child for name, child in comp.children if child.script \
								and getattr(child.script, "buttonStyle", None) == Button.RADIOBUTTON_STYLE \
	 							and child.script.groupName == groupName ]
...
class ButtonVisualState( VisualState ):
	...
	def onSave( self, dataSection ):
		...
		if self.iconTextureMapping:
			...
			self._writeMappingSection( mappingSection, self.iconTextureMappingType,
										self.iconTextureMapping )
	...
	def apply( self, componentScript ):
		...
		if hasattr( componentScript, "buttonIcon" ):
			...
			if self.iconTextureMapping:
				Utils.applyMapping( componentScript.buttonIcon,
										self.iconTextureMappingType,
										self.iconTextureMapping )
...
class Button( PyGUIBase, VisualStateComponent ):
	...
	def onLoad( self, dataSection ):
		...
		self.groupName = dataSection.readString( "groupName", None )
