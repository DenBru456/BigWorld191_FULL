...
class Seat( BigWorld.Entity ):
	...
	def prerequisites( self ):
		return [ Seat.modelNames[self.seatType] ]
	...
	# --------------------------------------------------------------------------
	def name( self ):
		return Seat.displayNames[ self.seatType ]
	...
	def set_seatType( self, oldType = None ):
		...
		#
		if not self.seatType in Seat.displayNames.keys():
			print methodName + "seatType set to illegal value."
			self.seatType = Seat.UNKNOWN

		#
		# Make sure the model is properly set.
		#
		if not self.seatType in Seat.modelNames.keys():
			self.model = None
			print methodName + "seatType has no model."
		else:
			self.model = BigWorld.Model( Seat.modelNames[ self.seatType ] )
			if self.model != None:
	...
	def moveToSitDownPosition( self ):
		...
		player.physics.seek( player.model.SitOnChair.getSeekInv(
				pos.x, pos.y, pos.z, self.yaw + math.pi ),
			5, 0.30, self.moveToSitDownPositionEnded )
	...
	def moveToSitDownPositionEnded( self, success ):
		...
		if not success or self.ownerID != 0:
			...
			BigWorld.callback( 1.0, self.cell.sitDownRequest )
...
def sitDown( entity ):
	...
	if lastKnownSeat.seatType == Seat.WOODEN_STOOL:
	...
	BigWorld.callback( entity.model.SitOnChair.duration,
		partial( sitDownEnded, entity ) )
