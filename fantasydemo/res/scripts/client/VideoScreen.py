...
class VideoScreen( BigWorld.Entity ):
	...
	def init( self ):
		try:
			...
			self.initCallback = BigWorld.callback(	random.uniform( 9, 11 ),
													self.init )
	...
	def changeFeed( self ):
		...
		self.feedIndex += 1
		if self.feedIndex >= len(VideoScreen.feedSources):
			self.feedIndex = 0
		feed = VideoScreen.feedSources[self.feedIndex]
		if feed != None:
			if self.webPage == None:
				self.webPage = BigWorld.WebPageProvider(1024,512,"")
