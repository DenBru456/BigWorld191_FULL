...
class Staff(Item):
	...
	guiIconNames = {
		...
		LIGHTNING:		"gui/maps/icon_items/icon_staff.tga",
	}
	...
	# Method: enactDrawn
	# Description:
	#	- Start the idle SFX going.
	# --------------------------------------------------------------------------
	def enactIdle( self, entity = None ):
		Item.enactIdle( self, entity )
		if self.spell.idleSFX != None:
			self.spell.idleSFX.attach( self.model )
			self.spell.idleSFX.go()
		if entity != None:
	...
	def unequip( self, entity ):
		...
		if entity != None:
			...
			if 0 in caps:
				caps.remove( 0 )
			entity.am.matchCaps = caps
		if self.spell.idleSFX != None and self.spell.idleSFX.source != None:
			self.spell.idleSFX.detach()
