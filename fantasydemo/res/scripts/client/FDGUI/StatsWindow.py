...
class StatsWindow( PyGUI.DraggableWindow  ):
	...
	def updateStats( self ):
		...
		self.component.drawnArea.text = '%s square km' % (formatNumber( loadedChunks / 100.0, 2 ),)
		...
		self.component.latency.text = "%s ms" % (formatNumber( float(BigWorld.getWatcher( "Comms/Latency" )) * 1000 ),)

		self.component.bandwidthToServer.text = "%s kbps" % (formatNumber( float( BigWorld.getWatcher( "Comms/bps out" ) ) / 1024, 1 ), )
		self.component.messagesToServer.text = formatNumber( BigWorld.getWatcher( "Comms/Messages out" ), 1 )
		self.component.bandwidthFromServer.text = "%s kbps" % ( formatNumber(float( BigWorld.getWatcher( "Comms/bps in" ) ) / 1024, 1 ), )
		self.component.messagesFromServer.text = formatNumber( BigWorld.getWatcher( "Comms/Messages in" ), 1 )

		maxBandwidth = int(BigWorld.getWatcher( "Comms/Max bandwidth per second" ))
	...
	def onBandwidthSlider( self ):
		value = int(self.component.bandwidthSlider.script.value) * 1024
		BigWorld.setWatcher( "Comms/Max bandwidth per second", str(value) )
