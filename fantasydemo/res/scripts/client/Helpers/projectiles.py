...
def destroyFireball( owner, fireball, fx, explosionSFXName, targetHitCallback, prereqs ):
	...
	fireballfx = sfx.bufferedOneShotSFX( explosionSFXName, explosion, explosion, lambda: owner.delModel(explosion), 10.0, prereqs )
...
def createFireball( projectileSFXName, explosionSFXName, source, target, srcOffset = None, targetHitCallback = None, prereqs = None ):
	fireball = BigWorld.Model("")	
	fx = sfx.PersistentSFX(projectileSFXName,prereqs)
	fx.attach(fireball)
	callback = partial( destroyFireball, source, fireball, fx, explosionSFXName, targetHitCallback, prereqs )
	global projectileSpeed
