...
class Channel( BigWorld.Base ):
	...
	def register( self, registrant ):
		...
		self.broadcast( str( id ) + " has joined the group." )

	def deregister( self, id ):
		self.broadcast( str( id ) + " has left the group." )
	...
	def tellOthers( self, source, message ):
		...
		for key in self.members.keys():
			if key != source:
				print "Telling", key
				self.members[ key ].tellClient( 2, str(source), message )
				print "Done"
	...
	def list( self, id ):
		self.members[ id ].tellClient( 2, "List", str( self.members.keys() ) )
