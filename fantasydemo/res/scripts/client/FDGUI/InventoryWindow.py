...
class DraggedItem( DraggableWindow ):
	...
	def handleMouseButtonEvent( self, comp, key, down, modifiers, pos ):
		if key == Keys.KEY_LEFTMOUSE and not down:
			self.active( False )

		return DraggedItem.base.handleMouseButtonEvent( self, comp, key, down, modifiers, pos )
...
class ItemDropRegion( PyGUIBase ):
	...
	def handleDropEvent( self, comp, pos, dropped ):
		...
		if dropped.script.parent.inventoryMgr:
			dropped.script.parent.inventoryMgr.selectItem( dropped.script.item['serial'] )
			dropped.script.parent.inventoryMgr._entity._checkStowAndEquip()
			dropped.script.parent.inventoryMgr._entity.dropTry()
			return True
...
class InventorySlot( Window ):
	...
	def handleMouseClickEvent( self, component, position ):
		InventorySlot.base.handleMouseClickEvent( self, component, position )
		if self.item:
	...
	def handleDragStartEvent( self, comp, pos ):
		...
		if self.item != None:
			...
			self.draggedItem.script.startDragging()
			return True
	...
	def handleDragStopEvent( self, component, position ):
		if self.draggedItem:
			self.draggedItem.script.active( False )
			self.draggedItem = None
	...
	def handleDropEvent( self, comp, pos, dropped ):
		...
		if self.parent.inventoryMgr is dropped.script.parent.inventoryMgr and self.parent.inventoryMgr != None:
			selfItem = self.item
			...
			return self.parent.handleDropEvent( comp, pos, dropped )
	...
	def setItem( self, item ):
		...
		if self.item:
			...
			self.component.textureName = Item.resolveIconName( itemType )
...
class InventoryWindow( DraggableWindow ):
	...
	def handleDragEnterEvent( self, comp, pos, dragged ):
		...
		return DraggableWindow.handleDragEnterEvent( self, comp, pos, dragged )
	...
	def handleDropEvent( self, comp, pos, dropped ):
		...
		if self.inventoryMgr != None and isinstance( dropped.script.parent, TraderWindow.TraderWindow ):
			traderWindow = dropped.script.parent
			itemIndex = traderWindow.inventoryItems.index( dropped.script.item )
			self.inventoryMgr._entity.onBuyItem( itemIndex )

			return True

		return DraggableWindow.handleDropEvent( self, comp, pos, dropped )
