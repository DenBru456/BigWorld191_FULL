...
class AvatarCommon:
	...
	# We could look at implementing some methods only in C++.

	def inviteToGroup( self, id ):
		self.internalInviteToGroup( id )

	def leaveGroup( self ):
		self.internalLeaveGroup()

	def tellClient( self, type, src, msg ):
		self.client.message( type, src, msg )

	def groupList( self ):
		if self.group:
			self.group.list( self.id )
		else:
			self.client.message( 3, "", "Not currently in a group" )
