...
class GuardNavigationException( Exception ):
	def __init__( self, id, sourceLocation, destination ):
		self.id = id
		self.sourceLocation = sourceLocation
		self.destination = destination

	def __str__( self ):
		return repr( self.id, self.link, self.destination )
...
class Guard(Avatar):
	...
	def __init__( self ):
		...
		if self.timeToLive > 0.0 and Guard.ENABLE_PATROL_TESTING != 1:
			self.addTimer( self.timeToLive, 0, Guard.DEATH_TIMER )

		self.armourColours = self.generateArmourColours()
		self.skinColours = self.generateSkinColours()
	...
	def handleHit(self, shooterId):
		...
		if(random.random() > 0.5):
			...
			self.otherClients.chat( Guard.alertLines[ randomLine ] )
	...
	def chooseTarget(self):
		...
		# If we have a camp spot, and we are too far from it, forget about our hateList.
		if self.campRadius and distance(self.campPosition, self.position) > self.campRadius:
			self.target = -1
			self.hateList = {}
			return
	...
	def think(self):
		try:
			...
			if self.scaredTime > BigWorld.time():
				...
				d = distance(entity.position, self.position)
...
#		self.otherClients.scanForTargets(0.0, 0.0, 0.0)
		...
		self.otherClients.chat( Guard.deathLines[ randomLine ] )
	...
	def moveToPosition(self, position, velocity, maxMove = MAX_MOVE):
		...
		try:
			if Guard.ENABLE_PATROL_TESTING:
				# increase search range for testing
				self.controllerId = self.navigateStep(position, velocity, maxMove, -1.)
			else:
				self.controllerId = self.navigateStep(position, velocity, maxMove)

		except:
			# A lot of the time if self.navigateStep fails then navigating to
			# self.position will get things going again.
			try:
				if Guard.ENABLE_PATROL_TESTING:
					# increase search range for testing
					self.controllerId = self.navigateStep(self.position, velocity, maxMove, -1.)
				else:
					self.controllerId = self.navigateStep(self.position, velocity, maxMove)

			except:
				print "Guard stuck", self.id, self.position
...
#		self.otherClients.scanForTargets(0.0, 0.0, 0.0)
	...
	def newFleePos(self):
		try:
			self.fleePos = BigWorld.findRandomNeighbourPoint( self.spaceID, self.position, 100 )
		except:
	...
	def doFlee(self):
		if self.fleePos is None or distance(self.position, self.fleePos) < 10.0:
			self.newFleePos()
	...
	def setDestinationAndSpeed( self ):
		...
		if Guard.ENABLE_PATROL_TESTING:
			...
			try:
				...
				e = GuardNavigationException( self.id, self.position, destinationPoint )
				raise e
		...
		try:
			if Guard.ENABLE_PATROL_TESTING:
				# increase search range for testing
				destination = self.canNavigateTo( destinationPoint, -1. )
			else:
	...
	def doPatrol(self):
		...
		try:
			...
			if self.nextPatrolNode != None:
				...
				print "Guard reached a dead end in node", newLastPatrolNode.guid, "at position", self.position
		except BigWorld.UnresolvedUDORefException:
			self.doWait( random.uniform( 9, 11 ) )
			return
		except GuardNavigationException, e:
	...
	def doCamp(self):
		d = distance(self.position, self.campPosition)
	...
	def gotoEntity(self, entity, velocity):
		...
		collideOffset = Math.Vector3( 0, 1.0, 0 )

		collideResult = BigWorld.collide(	self.spaceID,
											entity.position + collideOffset,
											nearPos + collideOffset )
		if collideResult:
			nearPos = collideResult[0] - collideOffset

		if distance(self.position, entity.position) > 3.0:
	...
	def gotoOwner(self):
		...
		if not owner or owner.isDead() or \
			( distance(self.position, owner.position) > Guard.FORGET_RANGE \
	...
	def transferHateList(self):
		...
		for id in self.visibleList:
			...
			if isinstance(e, Guard) and e.modelNumber == self.modelNumber:
				d = distance(e.position, self.position)
	...
	def startVisionSystem( self ):
		...
		self.visionController = self.addVision( self.VISION_FOV, self.VISION_RANGE, self.VISION_HEIGHT, self.VISION_PERIOD)
	...
	@staticmethod
	def generateSkinColours():
		return Guard.generateColoursFromRanges( GuardData.SOURCE_SKIN_COLOURS )


	@staticmethod
	def generateArmourColours():
		return Guard.generateColoursFromRanges(GuardData.SOURCE_ARMOUR_COLOURS)
...
BigWorld.addWatcher( 'entityTypes/Guard/patrolVelocity', getPatrolVelocity, setPatrolVelocity )
...
BigWorld.addWatcher( 'entityTypes/Guard/marchVelocity', getMarchVelocity, setMarchVelocity )
