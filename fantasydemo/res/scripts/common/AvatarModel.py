...
def create( unpackedModelData, model = None ):
	...
	if model:
		...
		model.saveActionQueue( savedActionQueue )
		model = BigWorld.Model( *unpackedModelData['models'] )
		model.restoreActionQueue( savedActionQueue )
	else:
		model = BigWorld.Model( *unpackedModelData['models'] )
...
def pack( persistentModelData ):
	...
	'''
	modelData = {}
	modelData['models'] = [AvatarModelData.MODEL_INDEXES[resPath] for resPath in persistentModelData['models']]
